...
/**
 * Changes by Thomas Fellinger [TF] https://www.netzgetaltung.at
 * [TF: 2018-07-29] Removed Screenname Argument at function getTweets()
 * [TF: 2018-07-29] Switched oAuth Lib to TwitterAPIExchange at function oauthGetTweets()
 * [TF: 2018-08-01] Add tweet_mode=extended to get all media entities
 */
...
if (!class_exists('TwitterOAuth')) {
  ...
  define('PESF_USING_EXISTING_LIBRARY_TWITTEROAUTH',true);
}
*/
if (!class_exists('TwitterAPIExchange')) {
  require_once('oauth/TwitterAPIExchange.php');
} else {
  define('PESF_USING_EXISTING_LIBRARY_TWITTEROAUTH',true);
}
...
class StormTwitter {
  ...
  /**
   * [TF: 2018-07-29] Removed Screenname Argument
   * Screenname now is set at class construct in defaults['screenname']
   */
  function getTweets($count = 20,$options = false) {
    ...
    $default_options = array('trim_user'=>true, 'exclude_replies'=>true, 'include_rts'=>false, 'tweet_mode' => 'extended', 'extended_tweet' => 'full_text');
  ...
  /**
   * [TF: 2018-07-29] Switched oAuth Lib to TwitterAPIExchange
   * TwitterOAuth did return an empty array, dont know why
   * Changed:
   * - new $connection_config var instead of $key, $secret, $token and $token_secret
   * - added $connection_url, $connection_method, $connection_getField
   * - replaced vars at error messages
   */
  private function oauthGetTweets($screenname,$options) {
    $connection_config = array(
      'consumer_key' => $this->defaults['key'],
      'consumer_secret' => $this->defaults['secret'],
      'oauth_access_token' => $this->defaults['token'],
      'oauth_access_token_secret' => $this->defaults['token_secret'],
    );
    $connection_url = 'https://api.twitter.com/1.1/statuses/user_timeline.json';
    $connection_method = 'GET';
    $connection_getField = '?screen_name=' . $screenname . '&tweet_mode=' . $options['tweet_mode'];
    
    $cachename = $screenname."-".$this->getOptionsHash($options);
    
    $options = array_merge($options, array('screen_name' => $screenname, 'count' => 20));
    
    if (empty( $connection_config['consumer_key'])) return array('error'=>'Missing Consumer Key - Check Settings');
    if (empty($connection_config['consumer_secret'])) return array('error'=>'Missing Consumer Secret - Check Settings');
    if (empty($connection_config['oauth_access_token'])) return array('error'=>'Missing Access Token - Check Settings');
    if (empty($connection_config['oauth_access_token_secret'])) return array('error'=>'Missing Access Token Secret - Check Settings');
     ...
     * TwitterOAuth did return an empty array, dont know why
     */
    $connection = new TwitterAPIExchange($connection_config);
    $result = json_decode($connection->setGetfield($connection_getField)->buildOauth($connection_url, $connection_method)->performRequest());
